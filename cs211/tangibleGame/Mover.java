...
class Mover extends PApplet{
  ...
  void update() {
    ...
    gravity.z = -sin(TangibleGame.rotX) * GRAVITY_CONSTANT;
  ...
  void checkCylinderCollision(){
      for(Cylinder c: TangibleGame.cylinders) {
        ...
        if(Cylinder.r1 + TangibleGame.BALL_RADIUS >= sqrt(pow(location.x - cX, 2) + pow(location.z - cZ, 2)) ) {
          TangibleGame.lastScore = TangibleGame.score;
          TangibleGame.score += sqrt(pow(velocity.x, 2) + pow(velocity.z, 2));
